...
- [x] **2.1 Cache 字段**：在 `Script/Core/game_type.py` 的 `Rhodes_Island` 中补齐医疗缓存：
  - 新增 `medical_patients_today`、`medical_hospitalized`、`medical_price_ratio`、`new_medical_price_ratio`、`medical_bed_limit`、`medical_inventory_accumulator`、`medical_surgery_records`，并以 `medical_income_today/medical_income_total` 取代原 `cure_income*` 字段。
  - `new_medical_price_ratio` 存储待调整的收费系数，0 表示无待调整值，非 0 时在每日结算时应用并清空。
...
- [x] **3.9 分诊策略**：在 `clinic_patient_management._select_triage_candidate()` 中将默认的 `normal` 策略调整为“危重优先 + 队列先来后到”，仅当候选队列不存在危重病人（严重度编号 3）时才按刷新顺序返回首位病人，其他策略保持原有排序逻辑。- [x] **3.10 收费系数延迟生效**：实现收费系数"第二天生效"机制：
  - `set_medical_price_ratio(price_ratio)` 将新系数写入 `new_medical_price_ratio`（待调整值），不直接修改当前生效系数。
  - `get_new_medical_price_ratio()` 读取待调整的收费系数，返回 0 表示无待调整值。
  - `apply_new_medical_price_ratio()` 在每日结算时调用，将待调整系数应用为当前系数并清空待调整值，同时重算床位上限。
  - 以上函数在 `clinic_patient_management.py` 中实现，并通过 `medical_service.py` 重导出。
## 4. 行为系统与 AI 接入
...
### 5.1 医疗部总面板 `medical_department_panel.py`
  ...
  2. 收费设置（ratio 调节按钮 + 预测）
     - 显示当前生效的收费系数与待调整系数（若有）
     - 调整时基于待调整系数（若存在）进行叠加计算，否则基于当前系数
     - 调整结果写入 `new_medical_price_ratio`，提示"明天生效"
  3. 医疗明细（病人列表、药物进度）
  4. 手术管理（候补病人、阻塞原因、日志）
  5. 财务报表（日/累计收入与消耗）
...
- [x] **6.1 settle_medical_department()**：在 `Script/System/Medical/medical_service.py` 中新增入口函数，内部按需调用 `clinic_patient_management.py` / `hospital_patient_management.py` / `hospital_doctor_service.py` 的工具方法，`update_base_resouce_newday()` 调用，负责：
  ...
  - 检查 `new_medical_price_ratio` 是否有待调整值，若有则调用 `apply_new_medical_price_ratio()` 将其应用为当前收费系数并清空待调整值。
  - 最后调用 `refresh_medical_patients()` 按新系数刷新病人列表。
- [x] **6.2 每日统计输出**：
